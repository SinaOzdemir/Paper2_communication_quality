# CR edit - i want the Google ngram figures not to include stopwords - line 112
...
#' }
#'
covars_make_baselines_CR <- function(x, ...) {
  UseMethod("covars_make_baselines_CR")
}


#' @rdname covars_make_baselines
#' @export
covars_make_baselines_CR.snippet <- function(x, ...) {
  covars_make_baselines_CR(x[["text"]], ...)
}

#' @rdname covars_make_baselines
#' @export
covars_make_baselines_CR.data.frame <- function(x, text_field = "text", ...) {
  covars_make_baselines_CR(x[[text_field]], ...)
}

#' @rdname covars_make_baselines
#' @export
covars_make_baselines_CR.corpus <- function(x, ...) {
  covars_make_baselines_CR(texts(x), ...)
}

#' @rdname covars_make_baselines
#' @export
covars_make_baselines_CR.character <- function(x, baseline_data = c("brown", "google"),
                                            baseline_year = 2000, baseline_word = "the", ...) {
  baseline_data <- match.arg(baseline_data, several.ok = TRUE)
  result <- NULL
  
  if ("brown" %in% baseline_data) {
    result <- c(result, make_baselines_brown_CR(x, baseline_word = baseline_word))
  }
  
  if ("google" %in% baseline_data) {
    result <- c(result, make_baselines_google_CR(x, baseline_year = baseline_year,
...
make_baselines_google_CR <- function(x, baseline_year, baseline_word) {
...
make_baselines_brown_CR <- function(x, baseline_word) {
  baseline_year <- NULL
  toks <- quanteda::tokens(quanteda::char_tolower(x), remove_punct = TRUE, remove_numbers = TRUE,
                           split_hyphens = TRUE)
  zero_length_tokens <- lengths(toks) == 0
