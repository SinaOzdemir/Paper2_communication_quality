...
ecemo <- read_rds("./data/corpii/EU_corpus_cleaned.RDS") %>% 
  select(text) %>% 
...
ggplot(head(emocount, 25), aes(x=count, y=reorder(emoji, count))) + 
  ...
  labs(title = "Most favorite supranational Emojis",
       x = paste0("Number of occurrences\nin ", nrow(ecemo), " Supranational tweets"),
       y = "")+
  theme_minimal()+
  theme(axis.text.y = element_blank())

ggsave(file = "./plots/DescriptiveOverview/Emojis.png", width = 16, height = 16, units = "cm")