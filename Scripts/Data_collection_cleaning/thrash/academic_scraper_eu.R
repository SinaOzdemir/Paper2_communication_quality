...
for (i in seq_along(eu_accounts)) {
  ...
  data.path<-paste0(getwd(),"/data/EU/json/",eu_accounts[i],"/")
...
for (i in 1:length(eu_accounts)){
 ...
 #some accounts don't exist anymore, so the loop throws an error
  ...
  try(expr = { data.path<-paste0(getwd(),"/data/EU/json",eu_accounts[i],"/")
  ...
  if(dir.exists(paste0(getwd(),"/data/EU/rds"))){
  
    saveRDS(tweet_data, file = paste0(getwd(),"/data/EU/rds/tweets_and_prof_info_",eu_accounts[i],".RDS"))
...
twitter_rds_cleaner(file = rds.files[1],
                    read = T,
                    save = T,
                    dim_even = T,
